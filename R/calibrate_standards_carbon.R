  ...
  if ("gain12C" %in% names(cal_df)) {
    ...
    if (nrow(ref_df$dlta13CCo2) > 1) {
      ...
      for (i in 2:nrow(ref_df$dlta13CCo2)) {
        ...
        # 2. at least 200 ~1Hz measurements available (filters out valve issues)
        ...
        if (!is.na(ref_df$dlta13CCo2$mean[i]) &
            ...
            abs(ref_df$dlta13CCo2$mean[i] - ref_df$dlta13CCo2Refe$mean[i]) < 5 &
          ...
          if (!(length(cal_id) == 0)) {
            ...
            cal_12C <- cal_df$gain12C[cal_id] * uncal_12C + cal_df$offset12C[cal_id]
            cal_13C <- cal_df$gain13C[cal_id] * uncal_13C + cal_df$offset13C[cal_id]
            
            if (!is.na(cal_df$r2_12C[cal_id]) & !is.na(cal_df$r2_13C[cal_id]) &
                cal_df$r2_12C[cal_id] > r2_thres & cal_df$r2_13C[cal_id] > r2_thres) {
              ref_df$dlta13CCo2$mean_cal[i] <- round(1000 * (cal_13C /
                                                               cal_12C / R_vpdb - 1), 3)
              ref_df$rtioMoleDryCo2$mean_cal[i] <- (cal_13C + cal_12C) / (1 - f)
            } else {
              ref_df$dlta13CCo2$mean_cal[i] <- ref_df$rtioMoleDryCo2$mean_cal[i] <- NA
            }
          } else {
            ref_df$dlta13CCo2$mean_cal[i] <- ref_df$rtioMoleDryCo2$mean_cal[i] <- NA
          }
    ...
    if (nrow(ref_df$dlta13CCo2) > 1) {
      for (i in 2:nrow(ref_df$dlta13CCo2)) { # use n-1 because the standards are bracketing
        ...
        if (!is.na(ref_df$dlta13CCo2$mean[i]) &
          ...
          if (!length(cal_id) == 0) {
            ...
            if (!is.na(cal_df$d13C_r2[cal_id]) & !is.na(cal_df$co2_r2[cal_id]) &
                cal_df$d13C_r2[cal_id] > r2_thres & cal_df$co2_r2[cal_id] > r2_thres) {
